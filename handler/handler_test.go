...
func TestWebServer(t *testing.T) {
	...
	value1, err := db1.GetKey("void")
	if err != nil {
		t.Fatalf("USA key error: %v", err)
	}
	want1 := "value-cyber"
	if !bytes.Equal(value1, []byte(want1)) {
		t.Errorf("Unexpected value of void key: got %q, want %q", value1, want1)
	}
	value2, err := db2.GetKey("kvdb")
	if err != nil {
		t.Fatalf("Soviet key error: %v", err)
	}
	want2 := "value-kvdb"
	if !bytes.Equal(value2, []byte(want2)) {
		t.Errorf("Unexpected value of kvdb key: got %q, want %q", value2, want2)
	}
}
