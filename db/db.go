...
// in bolt db a bucket is a collection of key-value pairs,
// much like a table in relational databases
var defaultBucket = []byte("default")
var replicaBucket = []byte("replication")

type Database struct {
	db       *bolt.DB
	readOnly bool //for database readonly replication
}
...
// if the database is in readonly mode it just retuns a error without setting up the value
func (d *Database) SetKey(key string, value []byte) error {
	if d.readOnly {
		return errors.New("read-only mode. you cannot write on a read only replica")
	}
...
// SetKeyOnReplica sets the key to the requested value into the default database
